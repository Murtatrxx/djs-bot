...
(async (query)  => {
	try {
		...
		if (!result) {
			return interaction.reply(`No result found for query \`${query}\`.`, { flags: 64 });
		}
		...
		interaction.reply(`${parts.join('\n')}`);
	} catch (e) {

	}
})(interaction.data.options.find(m => m.name === 'query').value)
    }
